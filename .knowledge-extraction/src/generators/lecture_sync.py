...
Lecture notes live in `notes/lectures/weekN/NNN-slug.md` and follow the template
in `notes/LECTURE-DISTILL.md`. They are the provenance layer organised by where
you actually were in the course; the topic references in `notes/` are the
durable layer organised by concept.

`notes/lectures/INDEX.md` is regenerated from the notes themselves, so it is
...
DEFAULT_LECTURE_DIR = REPO_ROOT / 'notes' / 'lectures'
DEFAULT_OUTPUT_DIR = REPO_ROOT / 'knowledge-base' / 'docs'
LECTURE_SUBDIR = 'lectures'
INDEX_FILENAME = 'INDEX.md'

SYNC_BANNER = (
    "<!-- Generated copy. Source: notes/lectures/{source}\n"
...
def generate_lecture_index(lecture_dir: Path | None = None) -> str:
    """Write `notes/lectures/INDEX.md` from the notes on disk. Returns its path."""
    lecture_dir = Path(lecture_dir) if lecture_dir else DEFAULT_LECTURE_DIR
