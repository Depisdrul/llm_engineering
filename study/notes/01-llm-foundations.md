...
|---|---|---|---|
| **Encoder-only** (BERT and *all* the embedding models in [file 05](./05-rag-and-vector-search.md)) | bidirectional | no | embeddings, classification, reranking |
...
- **Long-context Italian corpora hit the context ceiling ~35% sooner** than the equivalent English corpus. This matters when you size chunk counts and `k` in [file 05](./05-rag-and-vector-search.md).
