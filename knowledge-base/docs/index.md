# LLM Engineering Knowledge Base

*Personal study notes from Ed Donner's LLM Engineering course*

## Legal Disclaimer

This knowledge base contains personal study notes from the LLM Engineering course by Ed Donner. 
All content has been paraphrased and reorganized for personal reference.

**Original course**: [https://edwarddonner.com/](https://edwarddonner.com/)  
**Udemy course**: [LLM Engineering Course](https://www.udemy.com/course/llm-engineering-master-ai-and-large-language-models/)

No course materials are redistributed. All code examples are original implementations based on concepts learned.

For the actual course content, please enroll in the course.

---

## Navigation

### Topic References

Written from primary sources rather than lecture transcripts. Start here — these supersede the generated pages below.

- [LLM Foundations (Weeks 1-3)](reference/llm-foundations.md)
- [RAG & Vector Search (Week 5)](reference/rag-and-vector-search.md)
- [Training & Fine-Tuning (Weeks 6-7)](reference/training-and-finetuning.md)
- [Agents & Deployment (Week 8)](reference/agents-and-deployment.md)
- [API Keys & Runnability](reference/api-keys-and-runnability.md)
- [Lecture Distillation Workflow](reference/lecture-distillation-workflow.md)

### By Topic

#### Foundation Topics

- [LLM APIs & Integration](topics/01-foundations/llm-apis.md)
- [Development Tools](topics/01-foundations/dev-tools.md)

#### Core LLM Concepts

- [Prompt Engineering](topics/02-core-concepts/prompt-engineering.md)
- [Model Selection & Comparison](topics/02-core-concepts/model-selection.md)
- [Tokenization & Context](topics/02-core-concepts/tokenization.md)

#### Integration & Orchestration

- [Multi-Model Orchestration](topics/03-integration/multi-model.md)

#### RAG Systems

- [RAG Systems](topics/04-rag/rag-systems.md)
- [Embeddings](topics/04-rag/embeddings.md)
- [Vector Databases](topics/04-rag/vector-databases.md)

#### Agent Architectures

- [Agent Architectures](topics/05-agents/agent-architectures.md)
- [Tool Use & Function Calling](topics/05-agents/tool-use.md)
- [Multi-Agent Systems](topics/05-agents/multi-agent.md)

#### Fine-Tuning & Training

- [Fine-Tuning & Training](topics/06-training/fine-tuning.md)
- [Dataset Preparation](topics/06-training/dataset-prep.md)
- [Evaluation & Metrics](topics/06-training/evaluation.md)

#### GPU Computing

- [GPU Computing](topics/07-gpu/gpu-computing.md)

#### Production & Deployment

- [Production Deployment](topics/08-production/deployment.md)
- [UI & Interaction](topics/08-production/ui-interaction.md)

### Quick References

- [API Syntax](quick-ref/api-syntax.md)
- [Common Errors](quick-ref/common-errors.md)
- [Code Snippets](quick-ref/code-snippets.md)

### By Week

- [Week 1 Summary](week-summaries/week1.md)
- [Week 2 Summary](week-summaries/week2.md)
- [Week 3 Summary](week-summaries/week3.md)
- [Week 4 Summary](week-summaries/week4.md)
- [Week 5 Summary](week-summaries/week5.md)
- [Week 6 Summary](week-summaries/week6.md)
- [Week 7 Summary](week-summaries/week7.md)
- [Week 8 Summary](week-summaries/week8.md)

### Projects

- [Website Summarizer](projects/website-summarizer.md) - Week 1
- [Multi-Model Integration](projects/multi-model.md) - Week 2
- [RAG System](projects/rag-system.md) - Week 5
- [Price Is Right](projects/price-is-right.md) - Weeks 6-7
- [Autonomous Agents](projects/autonomous-agents.md) - Week 8

## Search

Use the search bar above to find topics, concepts, or code examples.

## About This Knowledge Base

This knowledge base is automatically generated from course notebooks and supplemented with manual notes. 
It's organized by topic rather than chronologically, making it easier to review concepts and find information.

*Last updated: 2026-08-17*
