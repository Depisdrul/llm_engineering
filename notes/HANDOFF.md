...
`notes/lectures/weekN/NNN-slug.md` file, filed here. See
...
| --- | --- | --- | --- |
| **Durable** | `notes/01-…08-*.md` | primary sources: arXiv, ACL Anthology, provider docs | highest — wins every disagreement |
| **Provenance (lectures)** | `notes/lectures/weekN/*.md` | Udemy transcript + slide deck | what the course claims, corrections flagged |
| **Provenance (notebooks)** | `knowledge-base/docs/topics/*` | generated from the `.ipynb` files | which notebook demonstrates what, and what it output |
...
```text
notes/                       ← hand-written, the durable + lecture layers
  ...
  lectures/
    INDEX.md                 ← generated
    weekN/NNN-slug.md        ← one per distilled lecture
    _slides/                 ← gitignored; decks are read from a Drive tab
.knowledge-extraction/       ← the pipeline
knowledge-base/
  nb2md.py, nb2mdREADME.md   ← the repo's single notebook parser
  docs/                      ← MkDocs site. ALL BUILD OUTPUT except index.md
```

`knowledge-base/docs/reference/*` and `docs/lectures/*` are copies of `notes/`,
overwritten on every run. Edit `notes/`.
...
so drafts can sit in `notes/` without appearing on the site. Lecture notes
...
```bash
python knowledge-base/nb2md.py . --recursive \
...
**nb2md is the single notebook parser.** `knowledge-base/nb2md.py` is
...
which carries unrelated linting and knowledge-base changes.
...
cache is `.knowledge-extraction/.cache/extracted.json`, gitignored.
